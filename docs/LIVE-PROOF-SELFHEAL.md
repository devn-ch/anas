...
### Seventh pass (decision tree)

The fault-rooted decision tree (`docs/AHR-SELF-HEAL-DECISION-TREE.md`,
`test/self-heal/decision-tree.yaml`) ranked thirteen findings; this pass closes nine of them at the
source, each with a regression test that fails on `cba1377` and passes on the new code. The stunt
node was NOT used (a PVE upgrade was running on it), so everything here is proven by unit, route
and harness tests against the captured rigs. The tree's own counters moved from **15 leaves with no
test, 5 with no code and 5 mis-applied actions** to **6, 4 and 0**.

- **F2 — LOSE-DATA — a repair that WROTE a proven-correct block was told to restore from backup.** RAID6 with rot in the target block AND in Q: P-XOR wins arbitration, the block goes through md and is provably right, and the post-check still counts the stripe because Q is wrong. The engine now re-reads the block COLD through the pin and arbitrates it again (`coldVerify`); if it passes, the outcome is `repaired` with an additive `parityResidual { array, band, bandIndex, mismatchCnt }` and "the block is repaired; the band still has a parity/Q mismatch — run Rewrite parity", never restore advice. The job rolls residuals into `AhrRepairResult.parityResiduals` in the same row shape a scrub's `parityMismatches` uses, the Scrubs parity indicator reads the newer of the pool's last scrub and its last repair, and `parityRewriteEvidence` accepts a repair-job source — so the residual is actionable without a fresh multi-hour two-phase scrub. *(`selfheal-repair.test.ts` on a RAID6 rig with Q rotten — repaired, `p-xor`, `postcheckMismatch 8`, the band named; `ahr-repair.test.ts` for the roll-up; `ahr-parity-rewrite.test.ts` + `routes/ahr-mutate.test.ts` for the evidence gate; harness for the indicator and the result panel.)*
- **F3 — FALSE ASSURANCE — every mapping failure claimed the bytes had passed their checksum.** `SelfhealMapError` now carries a `reasonCode` (`inline-extent` | `hole` | `owner-scan-truncated` | `band-unreadable` | `unresolvable`) set at each throw site, and the engine turns it into a new additive outcome kind, `not-examined`, counted in its own bucket (`AhrRepairResult.notExamined`) with the advice "this block could not be examined: &lt;reason&gt;; nothing was written; re-scrub after &lt;what would change&gt;". `mapping-abort` keeps its meaning for the one case the re-verify establishes — it READ the block and it passed. The arithmetic `repaired + unrepairable + aboveMd + mappingAbort + notExamined === blocks` holds in the schema, the notification and the UI. *(`selfheal-map.test.ts` per reason code; `selfheal-repair.test.ts` for inline/hole/band-unreadable end to end; `ahr-repair.test.ts` one case per code; harness for the panel.)*
- **F4 — RAID1 through-md rot read as "restore from backup" instead of "above md".** `reverify` aborted before the pre-check when every leg failed, so a mirror lost the diagnosis a parity band gets from the identical fault. The bounded check now runs first, because a mirror check compares the legs with each other: `mismatch_cnt == 0` means they AGREE and are both wrong (`above-md`, in the same words), `> 0` means they disagree and neither matches (`unrepairable`). *(`selfheal-repair.test.ts`, both branches, on the skewed-offset mirror rig.)*
- **F5 — "nothing written" after the block was written.** A `ForeignSyncOpError` from the POST-check hit the same catch as one from before the write. A `written` flag set at `writeDirect` now decides the sentence: "the block was written and matches its checksum; the post-check could not run (md started &lt;op&gt;) — re-scrub to confirm parity", reported `repaired` with an additive `postcheckSkipped` note. *(`selfheal-repair.test.ts`: a member fails between the write and the post-check; the block really is on the member and the outcome says so.)*
- **F8 — md's bad-block list existed only in the packaging hook.** `selfheal-map.ts` reads each member's `bad_blocks` (`md/rd<n>/bad_blocks`, the kernel's own symlink to `dev-<name>`) into the geometry, one range per line, matched in that member's own data coordinates; an unreadable list is `null` and is not the same as an empty one. Rewrite parity refuses a band with any non-empty list (`bad-blocks-present`), and the repair engine treats a member with a range over the target ROW as absent — RAID5 ends there with the reason, RAID6 still has the Q path. *(`selfheal-map.test.ts` for the parse and the coordinates; `selfheal-repair.test.ts` for absent-on-RAID5, empty-is-not-refusal, and a range elsewhere on the member; `ahr-parity-rewrite.test.ts` + the route for the refusal.)*
- **F9 — a read-back-guard failure advised a restore.** The guard fires precisely when nothing about the file has been established. It is `not-examined` / `unresolvable` now, with "the mapping and the array disagree; nothing was written, and nothing is known about this file's bytes". *(`selfheal-repair.test.ts`: the mirror whose md byte matches no leg.)*
- **F10 — neither Repair nor Rewrite parity was LUN-aware.** Repair consults `heldByLun` up front, one claims read for the whole request, and 409s `lun-session-active` before a confirm code exists when a path backs a LUN with a live initiator session — the block is proven and CoW leaves the old extent alone, but the initiator holds its own cache and has no idea the bytes moved. Rewrite parity WARNS instead: a band-level read patrol and a parity write, no file byte touched. *(`routes/ahr-repair.test.ts` against a materialised LIO capture, logged-in and idle; `routes/ahr-mutate.test.ts` for the disclosure's wording.)*
- **F11 — repair identity was the path alone.** `AhrRepairFile` gained an optional `inode`, the findings window sends it, and the engine `stat`s the path at the gates — before anything is pinned — and refuses a mismatch by name (`inode-changed`, in the `not-examined` bucket). *(`selfheal-repair.test.ts` both ways; the route proves the confirm signature binds it; the harness proves the window sends it.)*
- **F1 (the tooltip half) — the UI named a verb the operator cannot reach.** The Scrubs row indicator, the Rewrite-parity button reason and the parity window all said a mirror's legs "are arbitrated per block by Repair from parity", which needs a finding with named blocks that phase 2 cannot produce. They now read "mirror mismatch — not yet repairable from ANAS (selfheal.11); do not run md repair on a mirror". The verb itself is story `selfheal.11`. *(dialog-contracts harness, all three places.)*

Two tree findings this pass did NOT touch: **F6** (a DUP metadata correction leaves no signal ANAS
parses) and **F7** (`systemctl preset` re-enabling the mdcheck timers). Both are still open in
§Findings.

§Coverage and §The trees of the decision-tree doc are now GENERATED from the YAML by
