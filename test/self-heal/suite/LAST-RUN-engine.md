...
- date: 2026-09-14 05:25:10  node: anas-pve  kernel: 7.0.14-12-pve
...
| 7-txprobe | the tx-probe transaction touched no DATA chunk (the measured housekeeping set has no blind spot for the R1 assertion) | PASS | 51 band-A blocks measured, 0 in a data chunk |
| 7-scan | oracle scan (members of both arrays) located the segment-2 block on a band-B member | PASS | hit m3 of band B (loop9); verification-side map: m3 of /dev/md126, stripe 78, member offset 42995712 |
| 7-bandA-untouched | no DATA write on band A: every changed band-A sector is btrfs superblock/metadata housekeeping (the R1 assertion) | PASS | changed sectors: 100 — housekeeping=100 data=0 |
| 7-repair | two-band: REPAIR_CMD of the segment-2 (band B) marker block exits 0 using band B's geometry | PASS | rc=0 postcheck=0 n=4 (band B n=4) disk=m3 stripe=78 reason=/mnt/gtsh/@data/b1.bin block 120000 reconstructed from the XOR of the other 3 members of stripe 78 and verified against  |
| 7-member | band-B member block equals the original after repair | PASS | loop9@42995712 match=True |
| 7-bcheck | evicted bounded check over band B's stripe reads 0 | PASS | stripe 78 of md126: mismatch_cnt=0 |
