...
- date: 2026-09-15 04:18:41  node: anas-pve  kernel: 7.0.14-17-pve
...
| 5-disk | nothing was written: the rot's bytes are exactly the injector's (undisturbed) and every changed member sector is btrfs housekeeping (measured mirrors + non-DATA chunks) | PASS | rot at md@153665536 unchanged=True; changed sectors classified: housekeeping=66 data=0 content=0 |
...
| 8-inject | parity member's stripe row corrupted behind md (data members untouched) — the row on disk actually changed | PASS | data m0(loop0) scan-located at 4308992; rot injected on PARITY m4(loop4)@4308992, stripe 49; parity row 7763ca990ce3 -> b764cb0a3af8 |
...
| 7-txprobe | the tx-probe transaction touched no DATA chunk (the measured housekeeping set has no blind spot for the R1 assertion) | PASS | 35 band-A blocks measured, 0 in a data chunk |
| 7-scan | oracle scan (members of both arrays) located the segment-2 block on a band-B member | PASS | hit m0 of band B (loop6); verification-side map: m0 of /dev/md126, stripe 110, member offset 59817984 |
...
| 7-repair | two-band: REPAIR_CMD of the segment-2 (band B) marker block exits 0 using band B's geometry | PASS | rc=0 postcheck=0 n=4 (band B n=4) disk=m0 stripe=110 reason= |
| 7-member | band-B member block equals the original after repair | PASS | loop6@59817984 match=True |
| 7-bcheck | evicted bounded check over band B's stripe reads 0 | PASS | stripe 110 of md126: mismatch_cnt=0 |
...
| 9-neg-inject | both legs corrupted, with DIFFERENT junk on each | PASS | m0=20598e8eaee8 m1=4bc9364b26a1 @109232128 |
...
| 9-neg-untouched | both legs are byte-identical to the junk that was injected — the verb wrote nothing at all | PASS | m0 20598e8eaee8 -> 20598e8eaee8; m1 4bc9364b26a1 -> 4bc9364b26a1 |
...
| 8-no-evidence | the verb REFUSES without the evidence gate (exit 3, no-parity-mismatch) and issues no md action | PASS | rc=3 code=no-parity-mismatch reason=no completed scrub job is on record (this dev entry has no job queue) — pass --assume-mismatch to bypass the evidence ga; last_sync_action check -> check; parity row unchanged=True |
| 8-neg-inject | data-member rot injected below md | PASS | m4(loop4)@5160960 stripe 62: mismatch_cnt=8 |
| 8-neg | the verb REFUSES data rot (exit 3, data-corruption-found) instead of blessing it | PASS | rc=3 outcome=refused code=data-corruption-found reason=refused: data corruption found. Repair the data first. The fresh btrfs scrub of pool 'rig' reported: csum=1 |
