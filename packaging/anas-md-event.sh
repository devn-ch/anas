  ...
  Fail | FailSpare | DegradedArray | SparesMissing | RebuildFinished)
    ...
    if [ "$EVENT" = "RebuildFinished" ] && [ "$ACTION" = "check" ]; then
      ...
      if [ -n "$MISMATCHES" ] && [ "$MISMATCHES" -gt 0 ] 2>/dev/null; then
        NOTIFY_SEV=warning
        MSG="${MSG} md counted ${MISMATCHES} parity mismatch(es) during this check - the periodic scrub's phase 2 checks every file's checksum and will name any affected file; run Scrub now if you do not want to wait."
      ...
      if [ -n "$BADBLOCKS" ] && [ "$BADBLOCKS" -gt 0 ] 2>/dev/null; then
        NOTIFY_SEV=warning
        MSG="${MSG}. ${BADBLOCKS} unreadable sector range(s) are recorded on the array's members - data in those ranges could not be reconstructed during the rebuild. The periodic scrub's phase 2 checks every file's checksum and will name any affected file; run Scrub now if you do not want to wait."
